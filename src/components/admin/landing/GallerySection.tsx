      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" disabled={busy} onClick={() => ref.current?.click()}>
            {busy ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Plus className="h-4 w-4 mr-1" />} Upload images
          </Button>
        ...
        {editing && (
          <div className="rounded-md border border-border p-4 space-y-4 bg-muted/30">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold">Edit gallery image</h4>
            ...
            <Button onClick={save} disabled={busy}>{busy ? "Saving…" : "Save image details"}</Button>
          </div>
        )}
        {rows.length === 0 && <p className="text-muted-foreground text-sm">No gallery images yet.</p>}
