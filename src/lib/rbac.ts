...
export function can(roles: MaybeRole[] | MaybeRole, action: RbacAction, resource: string): boolean {
  const normalizedRoles = normalizeRoles(roles);

  if (action === RBAC_ACTIONS.MANAGE) {
    return normalizedRoles.some((role) => NAVIGATION_CONFIG.roles[role as AppRole]?.canManageRoles || role === "developer");
  }

  return canAccess(normalizedRoles, resource);
}
...
export { type AppRole, type SidebarItem } from "@/types/roles";
